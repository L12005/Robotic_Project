...
class BehaviorNode(Node):
    ...
    def _declare_and_load_config(self) -> ControllerConfig:
        return ControllerConfig(
            ...
            max_angular_speed=self.declare_parameter('max_angular_speed', 2.50).value,
            angular_gain=self.declare_parameter('angular_gain', 1.40).value,
            linear_gain=self.declare_parameter('linear_gain', 1.50).value,
            wait_duration=self.declare_parameter('wait_duration', 0.6).value,
            safety_radius=self.declare_parameter('safety_radius', 0.10).value,
            human_zone_time=self.declare_parameter('human_zone_time', 2.60).value,
            human_exit_time=self.declare_parameter('human_exit_time', 3.00).value,
            human_zone_half_width=self.declare_parameter('human_zone_half_width', 1.00).value,
            human_relevance_distance=self.declare_parameter('human_relevance_distance', 4.2).value,
            robot_body_size_x=self.declare_parameter('robot_body_size_x', 0.62).value,
            robot_body_size_y=self.declare_parameter('robot_body_size_y', 0.68).value,
            human_body_size_x=self.declare_parameter('human_body_size_x', 0.95).value,
            human_body_size_y=self.declare_parameter('human_body_size_y', 0.60).value,
            ...
            human_body_radius=self.declare_parameter('human_body_radius', 1.00).value,
